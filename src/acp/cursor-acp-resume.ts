#!/usr/bin/env bun
/**
 * Interactive Terminal resume for Cursor ACP sessions.
 *
 * CLI `agent --resume` only works for ~/.cursor/chats; ACP sessions live under
 * ~/.cursor/acp-sessions/<id> and must be resumed via JSON-RPC session/load.
 *
 * Usage:
 *   bun src/acp/cursor-acp-resume.ts --agent ~/.local/bin/agent --session-id <uuid> [--cwd <path>]
 */
...
function usage(): never {
  console.error(
    "usage: cursor-acp-resume.ts --agent <path> --session-id <id> [--cwd <path>]",
  );
  process.exit(2);
}
